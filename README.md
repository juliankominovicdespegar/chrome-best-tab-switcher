...
Screenshots are captured with `chrome.tabs.captureVisibleTab` when you **activate** a tab. Each capture is stored in memory for the open tab and persisted as a binary blob in **IndexedDB** (`tab-switcher` database), keyed by the tab’s exact URL. Closing a tab does not delete its thumbnail—you’ll still see it under **Recently closed**, in history results, and when you reopen the same URL.
...
Up to **100** URL thumbnails are kept; older entries are removed automatically (LRU by capture time). Existing thumbnails from the old `chrome.storage.local` backend are migrated automatically on first launch.
